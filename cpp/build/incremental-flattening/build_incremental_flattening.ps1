# Build and run incremental flattening reproduction (core stages)
# Usage:
#   .\build_incremental_flattening.ps1
#   .\build_incremental_flattening.ps1 -ModelPath "..\..\..\assets\Models2\torus_F7680.obj" -OutDir "..\..\..\assets\CP_Models\incremental_torus"
...
$exe       = Join-Path $outBinDir "incremental_flattening.exe"
...
$sources = @(
    (Join-Path $PSScriptRoot "incremental_flattening.cpp"),
...
Write-Host "Building incremental_flattening (core stages)..." -ForegroundColor Cyan
$compileArgs = @(
    "/EHsc", "/O2", "/std:c++17", "/MD",
    "/D_USE_MATH_DEFINES",
    "/Fe:$exe"
...
if (!$OutDir -and $ModelPath) {
    ...
    $OutDir = Join-Path $repoRoot ("assets\CP_Models\incremental_" + $name)
}

if ($ModelPath -and (Test-Path $ModelPath)) {
    if (!(Test-Path $OutDir)) { New-Item -ItemType Directory -Path $OutDir -Force | Out-Null }
    Write-Host "Running: $exe `"$ModelPath`" `"$OutDir`"" -ForegroundColor Cyan
    Push-Location $outBinDir
    & $exe $ModelPath $OutDir
    $code = $LASTEXITCODE
    Pop-Location
    if ($code -ne 0) { exit $code }
} else {
    Write-Host "Run manually:"
    Write-Host "  $exe path\to\mesh.obj path\to\output_dir"
}
