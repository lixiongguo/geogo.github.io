...
#if defined(FLUID3D_WASM) || defined(__EMSCRIPTEN__)
#define FLUID_LOG(...) ((void)0)
#else
#define FLUID_LOG(...) printf(__VA_ARGS__)
#endif
...
void FluidSim::advance(float dt) {
   ...
   while(t < dt) {
      ...
      if(t + substep > dt)
         substep = dt - t;
      FLUID_LOG("Taking substep of size %f (to %0.3f%% of the frame)\n", substep, 100 * (t+substep)/dt);
      
      FLUID_LOG(" Surface (particle) advection\n");
      advect_particles(substep);

      FLUID_LOG(" Velocity advection\n");
      //Advance the velocity
      advect(substep);
      add_force(substep);

      FLUID_LOG(" Pressure projection\n");
      project(substep); 
      ...
      //that of the object.
      FLUID_LOG(" Constrain boundary velocities\n");
      constrain_velocity();
...
void FluidSim::solve_pressure(float dt) {
   ...
   if(!success) {
      FLUID_LOG("WARNING: Pressure solve failed!************************************************\n");
   }
