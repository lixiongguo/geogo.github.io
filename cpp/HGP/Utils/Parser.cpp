...
bool Parser::loadOBJ(const char* filename, MeshBuffer* mb, MeshBuffer* smb)
{
    ...
    while (getline(ifs, line)) {
        ...
        if (token.compare("f") == 0) {
            ...
            iss >> n;
            if (!iss.fail()) mb->idx_pos.reserve(n);
        }
    }
...
bool Parser::loadVectorField(const char* filename, Mesh& cgalMesh)
{
    ...
    while (getline(fin, line)) {
        ...
        temp.str(line);
        temp >> token;
        if (token.compare("crossfield_angles") == 0) break;

        iss >> k1 >> k2 >> kv1x >> kv1y >> kv1z >> kv2x >> kv2y >> kv2z;

        if (cgalMesh.size_of_facets() == faceIndex) return false;
    ...
    while (getline(fin, line)) {
        ...
        if (!(check0 && check1 && check2)) return false;
#endif

        while (h->vertex()->index() != v1) {
            h = h->next();
            if (h == f->halfedge()) return false;
        }
        h->matching() = h1;
        h = h->next();
        h->matching() = h2;
