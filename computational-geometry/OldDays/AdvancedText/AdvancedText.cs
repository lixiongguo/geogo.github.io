...
namespace GOGUI
{
    ...
    public class AdvancedText : Text, IPointerClickHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        [System.NonSerialized]
        AdvancedTextImage m_CachedInputRendererBG;
        [System.NonSerialized]
        RectTransform caretRectTrans;
        [System.NonSerialized]
        RectTransform caretRectTransBG;
        //[System.NonSerialized]
        ...
        void CheckImage()
        {
            if (Application.isPlaying)
            {
                ...
                if (m_CachedInputRendererBG == null)
                {
                    GameObject goBG = new GameObject("ImageBG");
                    goBG.hideFlags = HideFlags.DontSave;
                    goBG.transform.SetParent(transform);
                    goBG.transform.SetAsFirstSibling();
                    goBG.layer = gameObject.layer;
                    caretRectTransBG = goBG.AddComponent<RectTransform>();
                    caretRectTransBG.anchorMin = Vector2.zero;
                    caretRectTransBG.anchorMax = Vector2.one;
                    caretRectTransBG.pivot = CachedRectTransform.pivot;
                    caretRectTransBG.anchoredPosition3D = Vector3.zero;
                    caretRectTransBG.sizeDelta = Vector2.zero;
                    caretRectTransBG.localScale = Vector2.one;
                    m_CachedInputRendererBG = goBG.AddComponent<AdvancedTextImage>();
                    m_CachedInputRendererBG.raycastTarget = false;
                }
        ...
        void GenerateText()
        {
            ...
            if (regenText)
            {
                ...
                vertImg.Clear();
                vertTxt.Clear();
                Vector3 spacing = lineSpacing * new Vector3(-0.08f, 0, 0);//1
                int worldNum = 0;
                for (int i = 0; i < textVertList.Count; i++)
                {
                    if (skip > 0)
                    {
                        ...
                        if (processed < 4)
                        {
                            ...
                            vert.position = old.position + offset + worldNum * spacing;
