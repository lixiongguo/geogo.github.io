...
## Git 提交含敏感信息后的补救 — `git reset --soft`（2025-05-09 更新）

### 问题场景

在提交文章更新时，不小心把包含 **AccessKeySecret** 的配置文件（如 `config.json`）一起 `git add` 并提交了。GitHub 的 push protection 检测到密钥后**拒绝推送**，但本地已经产生了多个 commit。

此时需要：**删除那几次"脏"提交，但保留所有文件修改**，然后用一个新的干净提交替代。

### `git reset --soft` 的作用

| 区域 | `--soft` | `--mixed`（默认）| `--hard` |
|------|----------|-----------------|----------|
| HEAD 指针 | ✅ 移动 | ✅ 移动 | ✅ 移动 |
| 暂存区 (staging) | ❌ 保留 | ✅ 清空 | ✅ 清空 |
| 工作区 (working dir) | ❌ 保留 | ❌ 保留 | ✅ 清空 |

简单来说：`--soft` **只移动 HEAD，不碰暂存区也不碰工作区**。所有修改保持在 staged 状态。

### 操作步骤

```bash
# 1. 将 HEAD 回退到远程分支，但保留所有本地修改
git reset --soft origin/develop

# 2. 此时所有修改仍在暂存区，只需添加真正要提交的文件
git add -A "_posts/"
git add ...

# 3. 用一个干净的新提交替代所有"脏"提交
git commit -m "文章更新"
...
- **`--hard`**：会丢弃所有文件修改！文章更新全部白干，❌
- **`--mixed`（默认）**：清空暂存区，修改变成 unstaged，需重新 `git add`，多一步操作
- **`--soft`**：修改保持在 staged，直接 `git commit`，最省事 ✅

### 一句话总结

> `git reset --soft` 是"撤销提交但不撤销改动"，让你把多个有问题的提交重新打包成一个干净的提交，同时保留所有文件修改。适用于从历史中清除敏感信息（如密钥）但保留代码改动的场景。
