...
可对照本系列 **《5.2 顾老师的 Semi-Discrete 解法》** 一文。
...
页面 **[最优传输算法演示](/optimal-transport.html)** 中 **「Merigot 半离散 OT」** 标签对应本文 **§4–§7** 的教学实现要点：

- 在 $[0,1]^2$ 上给定 $\rho$，用 **Lloyd** 在 $\rho$ 上迭代得到站点 $\{y_i\}$（对应 §8.1 的量化思想）；可选 **粗网格、少站点** 上预解 $w$，再 **最近邻延拓** 到细层作初值（§7.1）。  
- 每个像素归入使 $\|x-y_i\|^2-w_i$ 最小的 $i$，用 **像素求和** 近似 $\mu(\mathrm{Vor}^w)$ 与 $\Phi(w)$；这是 **几何 Power 图的像素近似**，边界为阶梯形，与 CGAL 等库中的 **精确多边形剖分 + 边界积分** 不同。  
- 一阶更新采用 **$\Phi$ 的梯度下降** 且每步对 $w$ **去均值**（§6.1）。

**「顾险峰半离散 OT」** 与 **「Merigot 半离散 OT」** 两标签共用同一套划分规则，区别在优化目标符号（$E$ 上升 vs $\Phi$ 下降）与 Merigot 侧的 Lloyd / 多尺度流程，便于对照阅读 5.2 与 5.3。
