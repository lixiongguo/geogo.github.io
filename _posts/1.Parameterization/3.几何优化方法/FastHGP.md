...
子空间构造完成后，需要在 $c_{\text{int}}$ 上找一个使边界/锥点三角形局部单射的系数。ATP（Alternating Tangential Projection）在低维 Jacobian 空间中求解。

### 7.1 局部凸化（Lipman frames）

以帧 $\zeta_i$（与 HGP 相同的 frame field，来自论文 [BZK09]）定义局部凸可行集 $B_i \subset \mathbb{C}^2$：
...
其中 $f_z^i, f_{\bar{z}}^i$ 是面 $i$ 的 Wirtinger 导数，$\mathrm{Re}(f_z/\zeta)$ 控制伸缩量，$|f_{\bar{z}}|$ 控制准共性（quasiconformality）程度。这两个凸不等式一起刻画了"接近共形、不折叠"的区域。

> **几何直觉**：$f_z$ 描述映射的保角伸缩，$f_{\bar{z}}$ 描述映射的"翻转"。$|f_{\bar{z}}| \leq \mathrm{Re}(f_z/\zeta)$ 等价于 Jacobian 行列式非负（不折叠）；加上 $\geq \sigma^2$ 的下界防止退化压缩。

### 7.2 交替切向投影

ATP 在两个空间之间交替投影：

1. **全局步**：面积加权最小二乘——在当前约化基 $H$ 的像空间中找到最接近当前局部可行点的全局映射
2. **局部步**：每个三角形独立投影到 $B_i$

初始化：$f_z^i = 1/\zeta_i$（沿 frame 方向单位伸缩），$f_{\bar{z}}^i = 0$（完全共形）。

收敛判据：法向量残差 < $10^{-4}$。若收敛，得到一个局部单射的调和初始映射；若不收敛（约 10-15% 情况），回归 HGP 的 SOCP 求解。

### 7.3 无锥多边界情形的简化

对 trivial holonomy 的多连通圆盘（无锥点），跳过 ATP，直接用 Tutte 型嵌入：主边界映射到圆，其余边界在邻域凸包内求解稠密线性系统——这是经典方法 [GGT06]，无需迭代。
...
5. **ATP** 在低维 Jacobian + 局部凸可行集中求初值；**Projected Newton** 在同样低维空间中优化扭曲
6. 速度约 HGP 的 10 倍，代价是鲁棒性略降（ATP 偶有不收敛）
