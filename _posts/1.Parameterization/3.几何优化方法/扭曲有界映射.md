...
> 本文整合两篇 SIGGRAPH 论文的核心理论与算法：
> **(1) BDHM** — *Bounded Distortion Harmonic Mappings in the Plane* (SIGGRAPH 2015)：单连通域 SOCP 优化；
> **(2) GLID** — *Globally Locally Injective Deformations*：多连通域 Newton-Eigen 框架。
...
| 维度 | BDHM (SIGGRAPH 2015) | GLID |
|------|---------------------|------|
| **拓扑** | 单连通域 | 多连通域（$$N$$ 个洞） |
| **调和分解** | $$f = \Phi + \Psi$$ | $$f = \widetilde\Phi + \widetilde\Psi + \sum \omega_i \ln|z-\rho_i|$$ |
| **边界积分** | $$\oint_{\gamma_0} f_z'/f_z = 0$$ | $$\oint_{\gamma_0} + \sum\oint_{\gamma_i} = 0$$ |
| **优化框架** | 凸化 → SOCP（锥规划） | 无约束 → Newton-Eigen |
| **约束处理** | 硬约束（active set + 二阶锥） | 能量自然屏障（对称 Dirichlet/\$\$1/\sigma_2^2\$\$） |
| **Hessian** | 不需要（SOCP 自动利用） | 解析推导 + PSD 闭式修正 |
| **局部单射** | 全局验证 → 加约束 | Lipschitz 不等式 + 幅角和证书 |
| **零空间** | 1 维 | $$4N+N+1$$ 维 |
| **收敛** | 1-3 轮 SOCP（外层） | Newton 迭代 + 线搜索 |
| **每次迭代计算量** | 求解 SOCP（中等） | Newton 步（快，仅需 O(n²) × 采样点） |
| **能量选择** | ARAP 为主 | 对称 Dirichlet / Exp Dirichlet / AMIPS |
