...
categories: [MeshDeform, TechRelated]
mathjax: true
---
...
**为什么算出来的一定是调和映射？** BDHM 的核心策略是"表示法"（representation approach）而非"约束法"：不是把 $$\Delta f = 0$$ 作为优化问题的一条约束去求解，而是直接在一个**由调和基函数张成的函数空间**中表示 $$f$$。具体来说：

1. 调和映射可以分解为 $$f(z) = h(z) + \overline{g(z)}$$（全纯+反全纯的共轭）
2. 用 Cauchy 重心坐标构造一组**天然的调和基函数** $$\{\Phi_k(z)\}$$ 和 $$\{\Psi_k(z)\}$$（它们本身满足 $$\Delta \Phi_k = 0$$、$$\Delta \Psi_k = 0$$）
3. 将映射表示为 $$f(z) = \sum_k \phi_k \Phi_k(z) + \overline{\sum_k \psi_k \Psi_k(z)}$$
4. 谐函数的线性组合仍然是谐函数 → 无论系数 $$\{\phi_k, \psi_k\}$$ 取何值，$$f(z)$$ 都自动是调和的

因此优化变量仅为系数 $$\{\phi_k, \psi_k\}$$，调和性被"硬编码"在了表示中，无需作为约束显式处理。
...
其中 $$h(z)$$ 和 $$g(z)$$ 是 $$\Omega$$ 上的全纯函数。这源于 $$\Delta f = 4 f_{z\bar{z}} = 0$$ 意味着 $$f_z$$ 是全纯的、$$f_{\bar{z}}$$ 是反全纯的。对应的复导数为：
...
**反过来也成立**：给定任意两个全纯函数 $$h, g$$，$$f(z) = h(z) + \overline{g(z)}$$ 自动是调和的。这就是 BDHM 的构造基础——用 Cauchy 重心坐标作为全纯基函数，其线性组合天然产生调和映射。
