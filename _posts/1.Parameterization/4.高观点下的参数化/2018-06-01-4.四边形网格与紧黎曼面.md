...
设 $q$ 在 $p_i$ 处有零点或极点（阶 $n_i$，正为零点、负为极点），除子为：

$$
(q) = \sum_i n_i \, p_i
$$

由于 $q \in H^0(S, K^{\otimes 4})$，全局亚纯截面的除子满足：

$$
\deg(q) = \sum_i n_i = 4(2g - 2) = 8g - 8
$$

一般地，若要求极点不超过除子 $D$，存在空间维数由 Riemann–Roch 给出：

$$
h^0(4K + D) - h^0(-3K - D) = \deg D + 4(1 - g)
$$
