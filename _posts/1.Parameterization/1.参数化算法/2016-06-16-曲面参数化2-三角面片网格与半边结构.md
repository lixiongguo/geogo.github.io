...
这个算法可以将单个边界的拓扑圆盘进行展平，为了简化处理我们下面用这个简化版("抽象")的人物头像来做实验
...
上面的算法虽然正确性是有保证的，但是缺陷也是很明显的。首先是我们需要先来指定一个凸多边形边界，那么如果原来的3D模型边界与我们所指定边界如果差别比较大的话，那就不可避免的会有比较大的扭曲变形，比较好的当然是算法根据3D模型的特征自动寻找一个比较符合的边界。
