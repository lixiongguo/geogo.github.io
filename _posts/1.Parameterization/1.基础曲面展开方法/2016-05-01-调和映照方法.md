...
$$
 ...
 Dirichlet能量也叫**膜能量**，它度量膜的拉伸变形程度，惩罚所有的扭曲变形。

<img src="https://lgximgs.oss-cn-beijing.aliyuncs.com/images/%2Fimage-20260605073614831.png" alt="image-20260605073614831" style="zoom:50%;" />
...
接下里我们要在离散的三角网格上定义调和映射，我们要研究的三角网格由一个个三角形构成，作用在其上的映射可以认为是分别作用在逐个三角形上，在每个三角形上都是一个简单的线性变换，所以可以认为我们要研究的映射是一个分段线性映射。接下来为了定义三角网格上的调和映射，我们先定义映射的梯度然后再定义其上的Laplace算子。
...
3. **求解**：将内部点方程与固定边界组合为一个稀疏线性系统：
   $$
   \mathbf{L} \mathbf{U} = \mathbf{0},\mathbf{u}|_{\partial_0} = \mathbf{g}\\
