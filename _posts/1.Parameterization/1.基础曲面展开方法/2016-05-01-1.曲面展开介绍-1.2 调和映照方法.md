...
极小化 Dirichlet 能量等价于求解 Laplace 方程 $\Delta u = 0, \Delta v = 0$——这正是**调和映照(Harmonic Mapping)**的定义。
...
求解调和映射（Harmonic Mapping），即极小化 Dirichlet 能量：
...
内部顶点均满足 $\Delta u = 0, \Delta v = 0$，L为网格 cotan 权重构建的**Laplace 矩阵**。
...
**Rado 定理**（1924）是在连续情况下**调和映射存在唯一性**的理论保证。
