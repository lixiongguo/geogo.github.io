...
Dirichlet能量的最小就是调和映射，这个在前面已经介绍了。共形能量在复分析与微分几何理论中有很多理论工具，所以会比较好求，我们会在后面详细讨论。其他能量的**梯度和Hessian矩阵不好求凸性也不好判定**，所以不能直接用非线性优化(NLP)的方法，只能针对不同的问题用各种数学技巧，定制不同的优化算法。下面我们将使用优化理论中的**交替优化方法(Local/Global)**来对等距能量进行迭代优化。



##  交替优化方法

以下方法主要来自于刘立刚老师于2008年发表的ARAP参数化方法，采用**迭代**优化的策略：先从一个简单算法（如 Tutte）初始化，然后交替进行如下两步——首先为每个三角形寻找一个尽量保持原形状的局部近似（**Local 优化**），再回头调整 Jacobian 矩阵使网格整体保持连接（**Global 优化**）。
