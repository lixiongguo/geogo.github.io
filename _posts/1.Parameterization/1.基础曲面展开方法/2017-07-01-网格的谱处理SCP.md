...
SCP（Spectral Conformal Parameterization，Mullen et al.）相比于 LSCM 的优点是**不必手工指定两个固定 pin 点**，从而避免 pin 点带来的额外扭曲。算法仍要求网格具有**非空边界**（拓扑上为圆盘片）；若曲面封闭，需要先切开得到边界（与 Geogram / libigl 等实现的前提一致）。
