...
如何控制三角网格(Mesh)上的分片线性映射$f$的**共形扭曲(记为$K$)**有界(即小于一个常数$C$)，并且同时确保$f$的局部单射性质。
...
具体来说，Lipman 将非凸的 ARAP 能量 $E_{ARAP} = \sum \|A_j - R_j\|_F^2$ 通过固定上一轮的旋转矩阵 $R_0$ 进行线性化，得到凸近似：
