---
layout: post
title: "数学理论-微分几何相关知识"
category: Math
---

### 曲率

如果要定义曲面的曲率首先就要先了解曲线的曲率

**曲线的曲率**

设曲线参数方程为 $ \mathbf{r}(s) $（$s$ 为弧长参数），则曲线在一点处的曲率 $ \kappa $ 定义为切向量 $ \mathbf{T} = \mathbf{r}'(s) $ 的转动速率：

$$
\kappa = \left\| \frac{d\mathbf{T}}{ds} \right\| = \|\mathbf{r}''(s)\|
$$

若曲线以任意参数 $t$ 给出 $ \mathbf{r}(t) $，则曲率公式为：

$$
\kappa = \frac{\|\mathbf{r}' \times \mathbf{r}''\|}{\|\mathbf{r}'\|^3}
$$

![image-20251124200526683](..\..\..\imgs\image-20251124200526683.png)

曲率半径 $ \rho = 1/\kappa $ 即为密切圆的半径。曲率越大，曲线弯曲越剧烈。

![image-20251124200551873](..\..\..\imgs\image-20251124200551873.png)

**Frenet坐标架**

对于空间曲线，在每一点可定义由三个互相正交的单位向量构成的 **Frenet 标架**：

- $ \mathbf{T} $：单位切向量 (unit tangent)
- $ \mathbf{N} $：主法向量 (principal normal)，指向曲线弯曲方向
- $ \mathbf{B} = \mathbf{T} \times \mathbf{N} $：副法向量 (binormal)

三者满足 **Frenet-Serret 公式**：

$$
\begin{aligned}
\mathbf{T}' &= \kappa \mathbf{N} \\
\mathbf{N}' &= -\kappa \mathbf{T} + \tau \mathbf{B} \\
\mathbf{B}' &= -\tau \mathbf{N}
\end{aligned}
$$

其中 $ \tau $ 为挠率。

![image-20251201164646558](..\..\..\imgs\image-20251201164646558.png)

**曲面的曲率**

可以证明曲面上曲率最大和最小的方向是一定是彼此正交的。

沿曲面某方向的**法曲率** $ \kappa_n $ 可由 **Euler 公式** 描述：

$$
\kappa_n(\theta) = \kappa_1 \cos^2\theta + \kappa_2 \sin^2\theta
$$

其中 $ \kappa_1, \kappa_2 $ 为主曲率，$ \theta $ 为该方向与 $ \kappa_1 $ 主方向的夹角。

![image-20251125194851646](..\..\..\imgs\image-20251125194851646.png)

**主曲率向量场**

曲面上的**主方向**是法曲率取极值的方向，对应主曲率 $ \kappa_1, \kappa_2 $。主曲率也可以用**形状算子（Shape Operator）**来刻画：形状算子 $ S: TM \to TM $ 是一个线性映射，满足：

$$
df(S\mathbf{X}) = dN(\mathbf{X}) \quad \forall \mathbf{X} \in TM
$$

其中 $ N $ 为单位法向量。形状算子 $ S $ 与 **Weingarten 映射** $ dN $ 本质上是同一概念——两者都描述了法向量沿切向变化的速率，区别仅在于 $ S $ 将结果表达为曲面上的切向量，而 $ dN $ 表达为 $ \mathbb{R}^3 $ 中的向量。

![image-20251124200643402](..\..\..\imgs\image-20251124200643402.png)

![image-20251124200839863](..\..\..\imgs\image-20251124200839863.png)

**曲率的计算**

主曲率是以下**广义特征值问题**的解：

$$
I\!I \begin{bmatrix} a \\ b \end{bmatrix} = \kappa \; I \begin{bmatrix} a \\ b \end{bmatrix}
$$

其中 $ I $ 为第一基本形式矩阵，$ I\!I $ 为第二基本形式矩阵，$ [a, b]^T $ 为切平面中的主方向。

![image-20251201163358084](..\..\..\imgs\image-20251201163358084.png)

**形状算子是矩阵**

形状算子（Weingarten 映射）在局部坐标下的矩阵表示为：

$$
S = I^{-1} \cdot I\!I
$$

其中：

$$
I = \begin{bmatrix} E & F \\ F & G \end{bmatrix}, \quad
I\!I = \begin{bmatrix} L & M \\ M & N \end{bmatrix}
$$

![image-20251201163225429](..\..\..\imgs\image-20251201163225429.png)

**如何计算主曲率**

展开矩阵 $ S = I^{-1} I\!I $ 得：

$$
I^{-1} = \frac{1}{EG - F^2} \begin{bmatrix} G & -F \\ -F & E \end{bmatrix}
$$

$$
S = \frac{1}{EG - F^2}
\begin{bmatrix}
GL - FM & GM - FN \\
- FL + EM & - FM + EN
\end{bmatrix}
$$

主曲率 $ \kappa_1, \kappa_2 $ 即为 $ S $ 的两个特征值。

![image-20251201163053241](..\..\..\imgs\image-20251201163053241.png)

### 曲面的第一基本型与第二基本型

第一基本量内蕴，第二基本量外蕴。

设曲面参数化为 $ \mathbf{r}(u,v) $，记偏导数 $ \mathbf{r}_u = \frac{\partial \mathbf{r}}{\partial u} $，$ \mathbf{r}_v = \frac{\partial \mathbf{r}}{\partial v} $，二阶偏导 $ \mathbf{r}_{uu}, \mathbf{r}_{uv}, \mathbf{r}_{vv} $。单位法向量为：

$$
\mathbf{n} = \frac{\mathbf{r}_u \times \mathbf{r}_v}{\|\mathbf{r}_u \times \mathbf{r}_v\|}
$$

**第一基本形式（First Fundamental Form）** 的系数为：

$$
E = \mathbf{r}_u \cdot \mathbf{r}_u,\quad
F = \mathbf{r}_u \cdot \mathbf{r}_v,\quad
G = \mathbf{r}_v \cdot \mathbf{r}_v
$$

![image-20251201163308087](..\..\..\imgs\image-20251201163308087.png)

第一基本形式写为：

$$
I = E\,du^2 + 2F\,du\,dv + G\,dv^2
$$

![image-20251201164353403](..\..\..\imgs\image-20251201164353403.png)

**第二基本形式（Second Fundamental Form）**的系数为法向量在曲面弯曲方向上的投影：

$$
L = \mathbf{r}_{uu} \cdot \mathbf{n},\quad
M = \mathbf{r}_{uv} \cdot \mathbf{n},\quad
N = \mathbf{r}_{vv} \cdot \mathbf{n}
$$

![image-20251201164413771](..\..\..\imgs\image-20251201164413771.png)

第二基本形式写为：

$$
I\!I = L\,du^2 + 2M\,du\,dv + N\,dv^2
$$

曲面在局部可以看作一个**二次曲面**（如椭球、双曲面等），而这些二次曲面的主轴天然就是正交的。可以参加libigl中computeCurvature的实现。

**脐点（Umbilic Point）** 是指两个主曲率相等的点，即 $ \kappa_1 = \kappa_2 $。在脐点处，所有方向都是主方向，切平面内的曲率是各向同性的。

![image-20251201170450415](..\..\..\imgs\image-20251201170450415.png)

**曲面上点的分类**（按主曲率符号）：

| 类型 | 条件 | 示例 |
|------|------|------|
| 椭圆点 (Elliptic) | $ \kappa_1 \kappa_2 > 0 $ | 球面上任意点 ($ \kappa_1 = \kappa_2 = 1/R $) |
| 双曲点 (Hyperbolic) | $ \kappa_1 \kappa_2 < 0 $ | 马鞍面 |
| 抛物点 (Parabolic) | $ \kappa_1 \kappa_2 = 0 $（其一为零） | 圆柱面上点 ($ \kappa_1 = 0, \kappa_2 = 1/R $) |
| 平点 (Planar) | $ \kappa_1 = \kappa_2 = 0 $ | 平面 |

![image-20251201170539146](..\..\..\imgs\image-20251201170539146.png)

---

### Gauss-Bonnet 定理

把一张平整的纸卷成圆筒，其弯曲程度似乎变了，但纸面上的内蕴几何性质却没有改变。**Gauss-Bonnet 定理**揭示的正是这一事实：曲面上每一点的高斯曲率在整张曲面上的积分，恒等于 $2\pi$ 乘以欧拉示性数 $\chi(S)$——即曲率的总和只与曲面的"洞数"有关，与具体的弯曲方式无关。

直观理解：篮球表面处处"向外鼓"，高斯曲率为正；马鞍中间"向内凹"，曲率为负；桌面曲率为 0。高斯曲率最神奇之处在于它是**内禀的**——蚂蚁在球面上爬，不用抬头看天空，光靠测量周围三角形内角和就能知道自己在正曲率世界（内角和 >180°）。

**定理推导**：将曲面剖分为 $N$ 个测地三角形，每个三角形的高斯曲率积分为其"内角亏量"——$\int_\Delta K\,dA = \alpha + \beta + \gamma - \pi$。将所有三角形的亏量累加，内部边上的外角相互抵消，顶点处三角形内角之和为 $2\pi$，最终化简为 $2\pi(V - E + F)$。而 $V - E + F = \chi(S)$ 正是欧拉示性数，因此：

$$\int_S K\,dA = 2\pi\chi(S)$$

用局部的小三角形内角偏差，推导出了全局的拓扑不变量——这是微积分与拓扑学之间最深刻的桥梁之一。

---

### 高斯曲率与平均曲率

前面计算出主曲率 $\kappa_1, \kappa_2$ 后，定义：

- **高斯曲率**：$K = \kappa_1 \kappa_2$
- **平均曲率**：$H = \frac{\kappa_1 + \kappa_2}{2}$

用形状算子 $ S $ 的迹与行列式表示：

$$
H = \frac{1}{2}\operatorname{tr}(S), \quad K = \det(S)
$$

![image-20251201163933128](..\..\..\imgs\image-20251201163933128.png)

此外，高斯曲率还有如下等价定义。

---

### 高斯映射（Gauss map）

曲面上每个点 $p \in S$ 存在唯一的单位法向量 $N(p)$，垂直于其切平面。将 $N(p)$ 平移到单位球面 $S^2$ 的对应位置，就得到了**高斯映射** $\varphi: S \to S^2$。

单位法向量在局部坐标系 $(u,v)$ 下由下式给出：

$$N(u,v) = \frac{x_u \times x_v}{\|x_u \times x_v\|}$$

![image-20251124184622415](..\..\..\imgs\image-20251124184622415.png)

**高斯映射的微分（Weingarten 映射）**：$\mathrm{d}N(X)$ 称为 Weingarten 映射（形状算子），它量化了法向量随曲面弯曲的变化速率：

$$\mathrm{d}g_p(v) = \frac{\partial N}{\partial v} = -W_p(v)$$

负号表示法向量的变化方向与曲面弯曲方向相反。在局部坐标下，Weingarten 映射的矩阵表示为 $W = I^{-1} \cdot II$，其中 $I$ 是第一基本形式矩阵，$II$ 是第二基本形式矩阵。

**例题**：计算旋转抛物面 $z = x^2 + y^2$ 在原点处的 Gauss 映照微分。

参数化曲面为 $x(u,v) = (u, v, u^2 + v^2)$。计算得：$x_u = (1,0,2u)$，$x_v = (0,1,2v)$。在原点处 $N(0,0) = (0,0,1)$，$I = \begin{bmatrix}1&0\\0&1\end{bmatrix}$，$II = \begin{bmatrix}2&0\\0&2\end{bmatrix}$，因此 $W = \begin{bmatrix}2&0\\0&2\end{bmatrix}$，$\mathrm{d}g_p$ 将切向量 $(a,b)$ 映射为 $(-2a, -2b)$。

**Gauss 曲率与 Gauss 映照的关系**：

- 在局部坐标下：$K = \frac{LN-M^2}{EG-F^2} = \det(W)$——Gauss 曲率就是 Weingarten 映射的行列式
- 几何意义：$K(p) = \lim_{A \to 0} \frac{\text{Area}(\varphi(A))}{\text{Area}(A)}$，即 Gauss 映照的面积放大率
- 当 $K>0$ 时 $\varphi$ 保持局部定向，$K<0$ 时反转定向，$K=0$ 时退化
- **$\int_R K\,\mathrm{d}A = \text{Area}(g(R))$**：曲面局部区域的总曲率等于其 Gauss 映照的像集面积
- **Theorema Egregium（高斯绝妙定理）**：高斯曲率完全由第一基本形式决定——曲率是内蕴的，与外部观察者视角无关

---

### 离散高斯曲率

甜甜圈内部是负曲率、外部是正曲率，根据 Gauss-Bonnet 定理，两者之和为零。

欧拉示性数：$ \chi = 2 - 2g $（$g$ 为亏格），环面的总角度亏量趋近于 0。

![image-20251028134522912](..\..\..\imgs\image-20251028134522912.png)

![image-20251125150744255](..\..\..\imgs\image-20251125150744255.png)

![image-20251125150756311](..\..\..\imgs\image-20251125150756311.png)

**离散内部点的高斯曲率**：

对于三角网格上的内部顶点 $ v_i $，其离散高斯曲率定义为 $ 2\pi $ 减去该顶点周围所有三角形的内角之和：

$$
K(v_i) = 2\pi - \sum_{v_j \in N_1(v_i)} \theta_j
$$

其中 $ \theta_j $ 为顶点 $ v_i $ 处各相邻三角形的内角（angle defect / 角度亏量）。

![image-20250318144531344](..\..\..\imgs\image-20250318144531344.png)

对于拓扑空间，度量决定了曲率，局部度量的放缩变化由共形因子决定。通过对度量进行放缩，可以将曲面共形地展平。

对于参数化算法，网格内部点的目标高斯曲率 $K^{new}$ 都是 0。

**内部点高斯曲率**：$ K(v_i) = 2\pi - \sum \theta_j $

![image-20250318162037612](..\..\..\imgs\image-20250318162037612.png)

**边界点测地曲率**：$ \kappa_g(v_i) = \pi - \sum \theta_j $

![image-20250318162131451](..\..\..\imgs\image-20250318162131451.png)

映射的共形性等价于对测度的放缩，局部形状的放缩变形仅由其放缩因子（scaling factor）决定。共形映射是几何内在属性，与其几何嵌入无关。

---

### Gauss-Bonnet 定理的离散化

对于带边界的离散曲面，Gauss-Bonnet 定理的完整形式为：

$$
\int_M K\,dA + \int_{\partial M} \kappa_g\,ds = 2\pi\chi(M)
$$

其中 $ \kappa_g $ 为边界上的测地曲率。对于无边界的闭曲面，边界项消失，退化为 $ \int_M K\,dA = 2\pi\chi(M) $。

![image-20251027161116351](..\..\..\imgs\image-20251027161116351.png)

对其直接离散化可推导出三角网格的高斯曲率：

$$
\sum_{v_i \in V} K(v_i) = 2\pi\chi(M) = 2\pi(V - E + F)
$$

其中 $ \chi = V - E + F $ 由 **Euler 多面体公式** 给出（$V$ 为顶点数，$E$ 为边数，$F$ 为面数）。

换言之，离散 Gauss-Bonnet 定理断言：所有顶点的**角度亏量之和**等于 $ 2\pi\chi $。当亏格 $ g = 0 $ 时（球面拓扑），总亏量 $ = 4\pi $。

![image-20251027161141161](..\..\..\imgs\image-20251027161141161.png)

---

### 方向导数与协变导数、LC联络

**定理 (Kobayashi)**：光滑曲面上的 **Levi-Civita 联络** 可以表示为球面上 Levi-Civita 联络通过 Gauss 映射的 **拉回 (pullback)**。

![image-20251115150448007](..\..\..\imgs\image-20251115150448007.png)

具有微分几何背景的读者应注意：**离散联络 (discrete connection)** 实际上编码的是 Levi-Civita 联络的**平行移动 (parallel transport)**，而非联络本身。

![image-20251115150502075](..\..\..\imgs\image-20251115150502075.png)
