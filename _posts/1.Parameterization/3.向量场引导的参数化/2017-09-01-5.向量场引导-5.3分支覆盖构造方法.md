...
$$
h(v)=\Big(\sum_{i=0}^{n} r_i\Big)\bmod 4,\qquad ls(v)=\frac{h(v)}{4}.
$$
...
$$
\vec X_{i,s}=\cos(s\pi/2)\,\vec d_i+\sin(s\pi/2)\,\vec d_i^\perp,\qquad
...
$$
L\,\tilde u = \mathrm{div}(\vec X),\qquad L\,\tilde v = \mathrm{div}(\vec Y).
$$
...
$$
\psi=\sum_j \Delta\mu_j\, h_j,\qquad \phi=\tilde\phi+\psi.
$$
