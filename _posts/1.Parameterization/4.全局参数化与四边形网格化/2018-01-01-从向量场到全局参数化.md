...
## 统一视角：从 cross field 到 seamless parameterization

MIQ、PGP、QuadCover 看起来分别来自混合整数优化、周期参数化和分支覆盖，但它们解决的是同一个核心困难：

> cross field 是一个带 $\mathbb Z_4$ 对称性的多值对象；全局参数化则需要把这个多值对象变成可积、可拼接、可量化的 $(u,v)$ 坐标。

因此从向量场到全局参数化，可以抽象成四步：

| 阶段 | 要解决的问题 | 离散对象 |
|:---|:---|:---|
| **方向传播** | 相邻面上的方向如何比较 | parallel transport / $\kappa_{ij}$ |
| **分支选择** | 跨边时需要转几个 quarter-turn | matching $r_{ij}$ 或 jump $p_e$ |
| **可积化** | 方向场是否能写成 $\nabla u,\nabla v$ | Hodge / Poisson / 最小二乘 |
| **周期量化** | 绕闭环后的参数差是否为整数格点 | period $\mu_j \in \mathbb Z^2$ |

三种方法的差异不在目标，而在“把这些离散结构放在哪里”：

- **MIQ**：把 jump 和整数平移放进优化变量，直接求解；
- **PGP**：把周期跳变吸收到目标梯度中，用周期参数域解释拼接；
- **QuadCover**：把 $\mathbb Z_4$ 多值性提升到 4-sheet covering space，在覆盖空间上做单值积分。
...
\text{cross field}
\;\longrightarrow\;
\text{quarter-turn jumps}
\;\longrightarrow\;
\text{integrable parameter functions}
\;\longrightarrow\;
\text{integer grid / quad layout}.
$$
...
> **参考论文**：Bommes, D., Zimmer, H., & Kobbelt, L. (2009). *Mixed-Integer Quadrangulation*. ACM Transactions on Graphics (SIGGRAPH), 28(3), Article 77.

Bommes 等人的 **MIQ** 是四边形网格化中最具代表性的 mixed-integer 框架。与 PGP 把整数跳变"藏"在目标梯度里不同，MIQ 把离散决策**显式写在边上**——每条内部边都是一个带旋转与平移的拼接接口。算法分两阶段：先在 cross field 上解 MIQP（上文已述），再求满足整数无缝约束的全局参数化，最后提取四边形网格。
...
> **注**：Phase 1 的 $p_e$ 与 QuadCover 的 matching $r_{ij} \in \{0,1,2,3\}$ 编码同一信息——相邻面之间需要旋转几个 quarter-turn 才能对齐（见 [分支覆盖构造全局参数化]({% post_url 2018-02-01-分支覆盖构造全局参数化 %}) §3.2.4）。

#### 1.2 初始方向场

Phase 1 需要一个初值 $\theta_f$。常见来源：

- **主曲率方向**：与几何特征对齐，四边形网格更贴合形状；
- **用户手绘 / 约束场**：交互式设计；
- **Zipper + N-RoSy 设计**（Kalberer et al.）：先固定奇异点指标，再构造拓扑合法的 period jumps。

本仓库实现中，初值取每面最长边在局部切平面上的投影角 `atan2`。
...
\boxed{
\begin{aligned}
\min_{u,v,\delta,r,\mathbf{t},\lambda} \quad & E(u,v,\delta,\lambda) \\[4pt]
\text{s.t.}\quad
& \begin{bmatrix} u_j \\ v_j \end{bmatrix}
= R(r_e\pi/2)\begin{bmatrix} u_i \\ v_i \end{bmatrix} + \mathbf{t}_e
\quad \forall\, e=(i,j) \\[4pt]
& \delta_T \in \{0,1,2,3\},\;
r_e \in \{0,1,2,3\},\;
\mathbf{t}_e \in \mathbb{Z}^2
\end{aligned}
}
$$

这是一个变量规模较大的 **MIQP**：连续变量为顶点 $(u_i, v_i)$ 和缩放 $\lambda_T$；整数变量分布在每条边上（$r_e$ 加 $\mathbf{t}_e$ 两个分量）和每个面上（$\delta_T$）。若将 $(U,V,\boldsymbol{p})$ 联合建模而不分阶段，整数维数与 UV 坐标相当，通用 MIP 几乎不可行——**分阶段 + 结构特化**是 MIQ 能实用的关键。
...
> **MIQ** 的哲学是"**把周期亮在约束方程中**"——跨边时显式地应用整数旋转与平移，每条边都是一个拼接接口，chart 边界就是变换非平凡的边。

与 QuadCover 的关系：二者对离散跳变的编码**同构**——QuadCover 的 matching $r_{ij}$ 与 MIQ 的边跳变 $p_e$ / $r_e$ 都是 $\{0,1,2,3\}$ 上的整数，描述相邻面之间 quarter-turn 对齐关系；区别在理论包装（branch cover + Hodge 分解 vs. mixed-integer 优化）和参数化求解路径。
