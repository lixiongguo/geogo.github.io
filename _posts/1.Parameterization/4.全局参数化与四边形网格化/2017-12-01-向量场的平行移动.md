...
$$
  ...
  在 Levi-Civita 联络上加一个**连接修正 $\varphi$（离散 1-form）**，使新联络在每个对偶环上的 holonomy 变成目标值。

- $K_v$：Levi-Civita 在顶点 $v$ 的离散 holonomy（角度亏缺）；
- $s_v$：我们指定的奇异点整数指标，要求满足 Gauss-Bonnet：

$$
\sum_v s_v = \chi(M).
$$

设目标 holonomy 为 $2\pi s_v$，我们要找一个连接修正 $\varphi$（离散 1-form）使得
$$
d\varphi = 2\pi s - K_v.
$$
