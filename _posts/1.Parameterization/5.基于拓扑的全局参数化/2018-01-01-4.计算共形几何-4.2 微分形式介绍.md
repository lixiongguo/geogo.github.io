...
微分形式是一套描述几何本质十分有效的语言，由陈省身的老师Cartan发明。它将局部分析对象与整体几何结构自然统一,更好地表达了几何模型的内在结构。许多几何量的本质并不依赖具体坐标，而应通过坐标无关的形式来刻画。
...
这一统一的 Stokes 定理将微积分中若干经典定理纳入同一个框架：

| 被积形式 $\omega$ | 区域维度 $k$ | 经典形式 |
|:---|:---|:---|
| 0-形式（函数 $f$） | 1-维曲线 | **梯度定理**：$\displaystyle\int_a^b \nabla f \cdot d\mathbf{r} = f(b) - f(a)$ |
| 1-形式（$\mathbf{F}\cdot d\mathbf{r}$） | 2-维曲面 | **旋度定理（经典 Stokes）**：$\displaystyle\int_{\partial S} \mathbf{F} \cdot d\mathbf{r} = \int_S (\nabla \times \mathbf{F}) \cdot d\mathbf{S}$ |
| 2-形式（$\mathbf{F}\cdot d\mathbf{S}$） | 3-维体 | **散度定理（Gauss）**：$\displaystyle\int_{\partial V} \mathbf{F} \cdot d\mathbf{S} = \int_V (\nabla \cdot \mathbf{F}) \,dV$ |

对应的外微分算子链：

$$
0\text{-形式}\ \xrightarrow{d}\ 1\text{-形式}\ \xrightarrow{d}\ 2\text{-形式}\ \xrightarrow{d}\ 3\text{-形式}
$$
...
，传统写法中包含四个复杂方程的麦克斯韦方程组，用微分形式可以极其简洁地写为两个方程：