...
弱对偶 $d^*\le p^*$ 恒成立。下面先分清两个方向，再给出**鞍点定理**对充分性的证明。
...
回忆 §对偶理论中的 (P)(D)。点 $(\boldsymbol{x}^*,\boldsymbol{\lambda}^*,\boldsymbol{\mu}^*)$（$\boldsymbol{\mu}^*\ge\boldsymbol{0}$）称为 $\mathcal{L}$ 的**鞍点**，若
...
由上两步，KKT 点 $(\boldsymbol{x}^*,\boldsymbol{\lambda}^*,\boldsymbol{\mu}^*)$ 是 $\mathcal{L}$ 的鞍点。代入 §鞍点定理：
...
```mermaid
flowchart LR
  KKT["KKT 条件"] --> SP["Lagrangian 鞍点 (SP)"]
  SP --> Strong["强对偶 p*=d*"]
  SP --> Global["x* 全局最优"]
  Convex["f,g 凸; h 仿射"] --> KKT
```
...
KKT 必要性（局部最优 $\Rightarrow$ 存在乘子）可在**一阶可行方向**语言下，由 **Farkas 引理**推出。证明只用到一阶 Taylor 展开，不依赖凸性。**充分性**（KKT $\Rightarrow$ 全局最优）见上节**鞍点定理**证明；Slater 用于必要性一侧保证强对偶与乘子存在。
