...
单值化定理保证**每个紧致黎曼面都存在常曲率度量**，但并未给出如何从给定度量构造它。**Ricci 流**提供了一条how的方法：沿流演化，曲率在全曲面上均匀化，最终收敛到与亏格相应的常曲率度量。

## 光滑 Ricci 流

**里奇流（Ricci Flow）**本质上是一种通过**演化度量**来调整曲面曲率的方法。在光滑二维黎曼流形 $(S, \mathbf{g})$ 上，设 $\mathbf{g}(t)$ 为随时间 $t$ 演化的**黎曼度量**，**Ricci 流**方程为：
$$
...
注意这里所说的圆形填充类似于Circle Pattern,但是Circle Pattern是定义在三角形上的外接圆，而这里的圆填充是定义在三角形顶点上的。要在三角网格上实现 Ricci 流，首先需要一种用有限数据**描述共形结构的离散度量**。
...
设 $\Gamma: V \to \mathbb{R}^+$ 为每个顶点分配半径 $\gamma_i$，$\Phi: E \to [0, \frac{\pi}{2}]$ 为每条边 $e_{ij}$ 分配锐角 $\Phi(e_{ij})$，称为**边的权重**函数这样的顶点和边的配置$(\Gamma, \Phi)$ 称为网格上的 **circle packing metric**。
...
<img src="https://lgximgs.oss-cn-beijing.aliyuncs.com/images/image-20260714094501912.png" alt="image-20260714094501912" style="zoom:50%;" />
...
![image-20260714185244816](/Users/lgxgeogo/Desktop/MyDoc/lixiongguo.github.io/imgs//image-20260714185244816.png)

对圆填充，$I(c_i, c_j) = \cos\phi_{ij}$。统一记号 $I_{ij} = \cos\Phi_{ij}$，则上节边长公式可写为
$$
l_{ij} = \begin{cases}
...
由此定义**反演距离圆填充度量** $(\Gamma, I)$，其中 $\Gamma = \{\gamma_i\}$，$I = \{I_{ij}\}$。**离散共形形变**仅改变半径 $\gamma_i$，保持 $I_{ij}$ 不变。

给定 $(\Gamma, I)$ 后，三角形内角由**余弦定理**给出，顶点曲率为角亏：
...
与单值化定理中按亏格选取的常曲率背景一致。固定反演距离后，所有使每个面上满足三角不等式的 $\mathbf{u}$ 构成 $M$ 的**容许度量空间**。该空间上的微分 $1$-形式 $\omega = \sum_i K_i\, du_i$ 是**闭形式**，从而**离散 Ricci 能量**
...
在连续情形下，**Chow (1991)** 证明了对正 Euler 特征数的闭曲面，若保持总面积不变，Ricci 流将收敛到常 Gauss 曲率度量。

在离散情形下，**陈-罗定理（Chow-Luo Theorem）**保证了**离散 Ricci 流的收敛性**，并能实现任意满足 Gauss-Bonnet 约束的目标曲率。
...
按单值化定理，$g\ge 2$ 的曲面**应在双曲空间 $\mathbb{H}^2$ 上展开**；强行映射到欧氏平面 $\mathbb{E}^2$ 会引入更大畸变。下图左为双曲空间展开，右为欧氏空间展开，对比可见前者更贴合曲面内蕴几何。
...
| 维度 | 单值化/几何化对象      | 典型背景                                           |
| :--- | :--------------------- | :------------------------------------------------- |
| 2D   | 紧致黎曼面             | $S^2,\;\mathbb{E}^2/\Lambda,\;\mathbb{H}^2/\Gamma$ |
| 3D   | 闭三维流形（分解片段） | 8 种 Thurston 几何                                 |

![image-20260714100618457](https://lgximgs.oss-cn-beijing.aliyuncs.com/images/image-20260714100618457.png)

## 参考文献

[1] X. Gu, F. Luo, S.-T. Yau. *Computational Conformal Geometry*. International Press, 2020.（顾险峰老师计算共形几何教材）

[2] M. Jin, J. Kim, F. Luo, X. Gu. *Discrete Surface Ricci Flow*. IEEE Transactions on Visualization and Computer Graphics, 14(5):1030–1043, 2008. [doi:10.1109/TVCG.2008.57](https://doi.org/10.1109/TVCG.2008.57).（球面 / 欧氏 / 双曲离散 Ricci 流统一数值框架；亦可用于全局参数化与曲面匹配）
