...
设 $\Gamma: V \to \mathbb{R}^+$ 为每个顶点分配半径 $\gamma_i$，$\Phi: E \to [0, \frac{\pi}{2}]$ 为每条边 $e_{ij}$ 分配锐角 $\Phi(e_{ij})$，称为边的权重函数。对 $(\Gamma, \Phi)$ 称为网格上的 **circle packing metric**。
...
两个 circle packing metric $(\Gamma_1, \Phi_1)$ 和 $(\Gamma_2, \Phi_2)$ **共形等价**当且仅当 $\Phi_1 \equiv \Phi_2$。共形形变即为**仅修改顶点半径而保持边交角不变**。

### 为什么是圆半径而非边长？

光滑曲面上的共形变换 $\mathbf{g} \to e^{2u}\mathbf{g}$ 只有一个标量场 $u$ 作为自由度。离散化时，顶点半径 $\gamma_i \approx e^{u_i}$ 恰好 $V$ 个自由度，维数匹配。反观边长度量有 $E \approx 3V$ 个自由度且绝大部分对应非共形形变：

1. **边长锁死角度**：给定三边，余弦定理唯一确定三个内角。任何边长改动必然改变角度——与"共形变换保角"的根本要求矛盾。边长同时耦合了"尺度信息"和"角度信息"，无法分离出纯共形因子。

2. **边长度量带约束优化**：需要每步满足三角形不等式，是庞大非线性约束集；而半径 $\gamma_i > 0$ 相互独立，是简单的无约束凸优化。

3. **曲率定义自然**：顶点 $v_i$ 的离散曲率用圆心角表达：
   $$
   K_i = 2\pi - \sum_{f \ni v_i} \theta_{f,i}
   $$
   $K_i$ 是 $\{\ln\gamma_j\}$ 的光滑函数，Ricci 流方程
   $$
   \frac{d\gamma_i}{dt} = (\bar{K}_i - K_i)\gamma_i
   $$
   对应一个凸能量函数的梯度流，全局收敛（Chow-Luo 定理）。
