...
## 内在联系：BFF 是 VSC 的“展开引擎”

这是最关键的联系！

- **VSC 负责回答：“在哪里切？”**
   它搜索最优切割图（cut graph），使得切开后的曲面**拓扑变为圆盘（disk）**。
- **BFF 负责回答：“切开后怎么展平？”**
   一旦切割完成（即得到一个带边界的单连通网格），**BFF 可立即用于计算高质量 UV 展开**。

> 💡 换句话说：VSC + BFF = **端到端**自动参数化 pipeline
>
> - VSC 自动生成最优切割；
> - BFF 对切割后的网格进行快速、保角展开。